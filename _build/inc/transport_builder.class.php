 ...
 *
 * 07.11.2013 - added functionality for TVs by Christian Seel (chsmedien.com)
 */
class TransportBuilder {
    ...
    public function make() {
        ...
        $this->log('Creating elements and adding them to the category');
    ...
    private function create_elements() {
        $elements = array();
        $snippets = (file_exists($this->src['data'] . 'data.snippets.php')) ? include($this->src['data'] . 'data.snippets.php') : array();
        $plugins = (file_exists($this->src['data'] . 'data.plugins.php')) ? include($this->src['data'] . 'data.plugins.php') : array();
        $tvs = (file_exists($this->src['data'] . 'data.tvs.php')) ? include($this->src['data'] . 'data.tvs.php') : array();
        ...
        foreach ($tvs as $tv) {
            $fields = array(
            	'id' => $tv['id'],
                'name' => $tv['name'],
                'caption' => isset($tv['caption']) ? $tv['caption'] : '',
                'description' => isset($tv['description']) ? $tv['description'] : '',
                'type' => $tv['type'],
                'default_text' => isset($tv['default_text']) ? $tv['default_text'] : '',
                'elements' => isset($tv['elements']) ? $tv['elements'] : '',
            );
            $elements[] = $this->new_element('modTemplateVar', $fields);
        }
        return $elements;
    }
